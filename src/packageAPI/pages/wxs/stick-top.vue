...
<template>
  ...
  const test = require("./stick-top.wxs");
    ...
    <onekit-scroll-view
      @scroll="theme=== 'dark' ? test.funcD :test.funcA"
    ...
    >
      <onekit-view class="page-banner">
        <onekit-image
          class="image"
          src="http://mmbiz.qpic.cn/mmbiz_jpg/TcDuyasB5T3Eg34AYwjMw7xbEK2n01ekiaicPiaMInEMTkOQtuv1yke5KziaYF4MLia4IAbxlm0m5NxkibicFg4IZ92EA/0?wx_fmt=jpeg"
          style="width:100%;height:200px"
          mode="aspectFit"
        ></onekit-image>
      </onekit-view>
      <onekit-view class="page-group-interaction page-group">
        <onekit-view class="page-nav-list">
      ...
      <onekit-block v-for="item in tabs" :key="item.title">
        <onekit-view class="goods-list" @:tap="handleClick">
          <onekit-image :src="item.img" mode="widthFix"></onekit-image>
          <onekit-view class="item-title">{{item.title2}}</onekit-view>
          <onekit-view class="item-desc">{{item.desc}}</onekit-view>
        </onekit-view>
      </onekit-block>
    </onekit-scroll-view>
  </page>
