...
Page({
  ...
  initTabs() {
    const currentPermissionIndex = ONEKIT.data.currentPermissionIndex
    const tabLength = ONEKIT.data.tabs[currentPermissionIndex].length
    const that = this
    wx.getSystemInfo({
      success(res) {
        that.setData({
  ...
  onPermissionChange(e) {
    const oldIndex = ONEKIT.data.currentPermissionIndex
  ...
  queryOneByOpenId(collection, openid, options = {
    ...
    const _openid = openid || ONEKIT.data.openid
    db.collection(collection).where({
      ...
      success: res => {
        ...
        if (resFirstData._openid && resFirstData._openid !== _openid) {
          ...
          if (showError) ONEKIT.showErrorModal('获取', err)
  ...
  updateOneByOpenId(collection, openid, data, options = {
    ...
    ONEKIT.queryOneByOpenId(collection, openid || '', {
      success: dbData => {
        if (dbData) { // 已有数据，进行更新操作
          db.collection(collection).doc(dbData._id).update({
            ...
            fail: err => {
              if (showError) ONEKIT.showErrorModal('设置', err)
        ...
        } else if (!openid || openid === ONEKIT.data.openid) { // 还没有插入过数据且要操作的是自己的数据，进行新增操作
  ...
  updateMyWhatsUp() {
    const data = {
      whatsUp: ONEKIT.data.myWhatsUp
    }
    ONEKIT.updateOneByOpenId('perm1', '', data, {
      showLoading: true,
      showError: true,
      success: () => {
        app.globalData.myWhatsUp = ONEKIT.data.myWhatsUp
        ONEKIT.setData({
          hasMyWhatsUp: true
        })
        wx.showModal({
  ...
  queryAdminWhatsUp() {
    ONEKIT.queryOneByOpenId('perm1', 'kiki', {
      showLoading: true,
      showError: true,
      success: data => {
        const content = (data && data.whatsUp) || ''
  ...
  updateAdminWhatsUp() {
    const data = {
      whatsUp: ONEKIT.data.adminWhatsUp
    }
    ONEKIT.updateOneByOpenId('perm1', 'kiki', data, {
      showLoading: true,
      showError: true,
      success: res => {
        if (res.updated === 0) {
          ...
          app.globalData.adminWhatsUp = ONEKIT.data.adminWhatsUp
          wx.showModal({
            content: '设置成功',
            showCancel: false
          })
  ...
  updateMyEmail() {
    const data = {
      email: ONEKIT.data.myEmail
    }
    ONEKIT.updateOneByOpenId('perm2', '', data, {
      showLoading: true,
      showError: true,
      success: () => {
        app.globalData.myEmail = ONEKIT.data.myEmail
        wx.showModal({
          content: '设置成功',
          showCancel: false
        })
  ...
  queryAdminEmail() {
    ONEKIT.queryOneByOpenId('perm2', 'popo', {
      showLoading: true,
      showError: true,
      success: data => {
        const content = (data && data.email) || ''
  ...
  updateAdminEmail() {
    const data = {
      email: ONEKIT.data.adminEmail
    }
    ONEKIT.updateOneByOpenId('perm2', 'popo', data, {
      showLoading: true,
      showError: true,
      success: () => {
        app.globalData.adminEmail = ONEKIT.data.adminEmail
        wx.showModal({
          content: '设置成功',
          showCancel: false
        })
  ...
  queryProduct() {
    ONEKIT.queryOneByOpenId('perm3', 'admin', {
      showLoading: true,
      showError: true,
      success: data => {
  ...
  updateProductPrice() {
    const data = {
      price: parseInt(ONEKIT.data.product.price, 10)
    }
    ONEKIT.updateOneByOpenId('perm3', 'admin', data, {
      showLoading: true,
      showError: true,
      success: () => {
        wx.showModal({
  ...
  queryServerData() {
    ONEKIT.queryOneByOpenId('perm4', 'server', {
      showLoading: true,
      showError: true,
      success: data => {
  ...
  updateServerData() {
    const data = {
      data: ONEKIT.data.serverData
    }
    ONEKIT.updateOneByOpenId('perm4', 'server', data, {
      showLoading: true,
      showError: true,
      success: () => {
        wx.showModal({
