  ...
  export default {
    ...
    methods: {
      showPicker() {
        ...
        eventBus.$emit('onekit-date-picker-change-done', onekit_e)
        this.parent.done();
      },
      cancel() {
      ...
      change(data) {
        ...
        if (this.columnText[0].column && this.columnText[1] && this.columnText[2]) {
          const y = this.columnText[0].column.date
          const m = this.columnText[1].column.date
          const d = this.columnText[2].column.date
          this.result = `${y}-${m}-${d}`
        }
